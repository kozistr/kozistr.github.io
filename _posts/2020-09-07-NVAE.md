...
> \sum_{l=2}^{L} $\mathbb{E}_{q(z_l\|x)} KL(q(z_{l}\|x, z_{<l})\|\|p(z_{l}\|z_{<l}))$
...
network 의 receptive field 크기를 늘릴 수록 long-range correlations 을 잡는데 유리하고 설명하는데, 
encoder / decoder 에 사용된 residual network 안에 convolution kernel size 를 늘리면 된다고 합니다.

그런데, 그냥 늘리면 computation 이 엄청 늘어나니까, 이걸 depthwise separable convolution 을 사용해 해결합니다.
...
처음에 언급했듯, BN 은 training instability 때문에 BN 대신 WN (Weight Normalization) 을 사용했는데,
