...
> $p(z_{l}^{i}\|z_{<l}) = \mathcal{N} (\mu_{i} (z_{<l}), \sigma_{i} (z_{<l})$ 가 $i^{th}$ variable in $z_{l}$ prior 가 normal 일 때,
...
그래서 이 연구에선 Lipschitz constant 를 regularizing 함으로 bounded 를 ensure 한다라 가정을 합니다.

이어서, Lipschitz constant 를 측정하기는 힘드니, *Spectral Regularization* 을 각 layer 에 적용을 합니다. (lipschitz constant 를 minimize 해 주는 scheme 에서)

loss term 에도 해당 regularization term 을 추가를 해서 minimize 합니다.

> $L_{SR} = \lambda \sum_{i} s^{(i)}$, $s(i)$ 는 $i^{t*}$ convolution 의 largest singular value
