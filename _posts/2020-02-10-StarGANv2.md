...
> $L_{adv} = E_{x,y}[log D_y(x)] + E_{x,y^~,z}[log (1 - D_y^~(G(x, s^~)))]$
...
> $L_{sty} = E_{x,y^~,z}[||s^~ - E_{y^~}(G(x, s^~))||_1]$
...
> $s_{i^~} = F_{y^~}(z_i), i \in {1, 2}$

> $L_{ds} = E_{x,y^~,z_1,z_2}[||G(x, s_1^~) - G(x, s_2^~)||_1]$
...
> $L_{cyc} = E_{x,y,y^~,z}[||x - G(G(x, s^~), s^)||_1]$
>
> $s^ = E_y(x)$
