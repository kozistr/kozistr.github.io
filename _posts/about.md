...
CV : [[PDF] (as of Nov. 2022)](http://kozistr.tech/cv.pdf)
...
- Captcha model to break captchas for the automation product.
  - Developed the lightweight model (text detector & captcha classifier) for inference in real-time (about `1000 TPS` for a batch transaction, `80 ~ 100 TPS` for a sample on the CPU) and built the RESTful API server to serve the model in real-time on the CPU.
